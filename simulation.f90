subroutine mcsimulation(data_input,statebasis,sdcsBasis,eldcs,dicsBasis,VarPs,datamc)
    ...
    use input_data	
	use ParticleDynamics	! contains input such as incident energy, benchmark mode
    use Ps_module
    ...
    logical:: diss
    real(dp):: cutoffEn, minExcEn, path
    ...
    do simIndex = 1, totalSims
       ...
       !Choose inital path length for incident particle. (z axis defined to be intial particle direction)
       !call selectPath(path,stateBasis,en_incident,0)
       !call update_position(particlebasis(partNum),path,1.0,0.0,0,datasim)
       !call update_time( 
	  ...
	  if(bmode .eqv. .true.) then ! Ps Formation Benchmark Simulation will be run instead of default simulation
		...
		call collisionsimulation(data_input,statebasis,sdcsBasis,dicsBasis,particlebasis,minExcEn,partNum,coll,simIndex,datasim,ionop,bmode,VarPs,eldcs,Elec)		
		...
		coll = coll + 1 ! Increment to consider the next collision					
             ...
             minExcEn = statebasis%b(2)%enex !Minmum excitation energy available for target species (H2)
             cutoffEn = minExcEn   !Cutoff energy for the simulation
	     ...
	     do while((particlebasis(partNum)%energy(coll).GE. cutoffEn) .AND. (particlebasis(partNum)%energy(coll) .LE. 700)) 				                               
	        particlebasis(partNum)%colls = coll ! Updates the amount of collisions the particle has gone through	
			datasim%collPerGen(particlebasis(partNum)%gen) = datasim%collPerGen(particlebasis(partNum)%gen) + 1	! Updates collisions per generation
			print*, 'Before collisionsimulation call', coll
			call collisionsimulation(data_input,statebasis,sdcsBasis,dicsBasis,particlebasis,minExcEn,partNum,coll,simIndex,datasim,ionop,bmode,VarPs,eldcs,Elec)
			print*, 'After collisionsimulation call', coll
			coll = coll + 1 ! Increment to consider the next collision		
	     end do
	  end if
...
subroutine collisionsimulation(data_input,statebasis,sdcsBasis,dicsBasis,particlebasis,minExcEn,partNum,coll,simIndex,datasim,ionop,bmode,VarPs,eldcs,Elec)
    use numbers
    use state_class        ! defines states (and basis of them) with operations on them
    use sdcs_module
    use totalcs_module     !  reading totalcs files
    ...
    integer:: stateNum
    type(dcs):: dcsaten
    real(dp):: cosangle, avang, minExcEn, randNum, phi, path	
    real(dp):: mass, deltaT
    ...
    if(bmode .eqv. .false.) then ! Run Default Simulation	
	   ...
	   !Commented out, time is now handled in E_Field
	   
       !particleBasis(partNum)%time(coll+1) = particlebasis(partNum)%time(coll) + deltaT
       call dissociation(particlebasis(partNum),statebasis,stateNum,dicsBasis,tcs,datasim,data_input,coll) 
	  ...
	  print*, datasim%tRes
          ...
          call update_energy(statebasis,sdcsBasis,eldcs,stateNum,tcs,particlebasis,minExcEn,partNum,cosangle,coll,ionop,data_input%enlossop,datasim,bmode,VarPs) ! Update the energy of the particle
	  call timeElapsed(particlebasis(partNum)%energy(coll),stateNum,tcs%CS(stateNum),VarPs,datasim)
