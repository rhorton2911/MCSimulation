...
subroutine populateStatesVcs(statebasis,tcsbasis)
    ...
    integer:: counter, tempCounter, Nmax, numElastic, numVibStatesTot
    ...
    integer:: vVal, inext
    integer:: inum, Nmaxall, vfVal, gridLength, halfInt, nfile, numLines, numDiss
    character*10::str1, str2, str3, str4, str5, str6, str7, str8, str9, str10
    character*10::str11, str12  !Used for file IO
    character*10::enString
    character*11::threshEn
                   ...
                   if (ii .eq. 1) then
                      ...
                      do while(.not. foundState)
                         if ((trim(adjustl(line(1:5))) .eq. trim(adjustl(newstates(jj)%stlabel))) .and. (vVal .eq. newstates(jj)%v)) then
                            ...
                            print*, "ERROR: could not state with given electronic and vibrational state label, stopping" 
...
subroutine get_csatein(statebasis,eincident,tcs)
  ...
  real(dp), dimension(statebasis%n):: ar_en,ar_cs, ar_df
