...
contains
  ...
  subroutine make_sdcs(self,Nenfine,enfine)
    ...
    do i=1,data_in%Nsdcs
       ...
       tmpstr = data_in%filename_sdcs(i)(9:)
...
! find the first point on the fine energy grid the first (lowest) SDCS is available
  ...
  subroutine get_sdcsatein(sdcsob,eincident,sdcsen)
    ...
    type(sdcs):: sdcsen
    real(dp):: e1, e2, w1, w2
    integer:: i, j, j1, j2, Ncgsdcs, nBelow, cutoffInd
    integer:: numPoints, ii, kk, jj, endInd, midInd
    ...
    if (int(cutoffEn/deltaE) .lt. 4.0) then
...
!
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine readSdcs(data_input, sdcsBasis, Nenfine, enfine)
      use input_data
...
50 continue
  ...
  subroutine intpSdcsModFine(data_input,sdcsBasis, Nenfine, enfine)
      ...
      real(dp), dimension(:)::enfine
      real(dp), dimension(:), allocatable:: ecgBelow
      ...
      real(dp)::eOutMin,eOutMax,step
      integer::numOutBelow, numSDCS, cutoffInd, numThousands
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   subroutine getSdcsDistAtEIn(sdcsob,eincident,sdcsen,tcsob,stateBasis)
       use totalcs_module
       use state_class
       implicit none
       ...
       call normaliseSDCSDist(tcsob,sdcsen,statebasis,eIncident)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    subroutine constructSdcsDist(tcsob,sdcsob,statebasis)
        use totalcs_module
        use state_class
        implicit none
        type(totalcs)::tcsob
        ...
        real(dp):: eOut, deltaE, cutoffEn, eIncident, eIon
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    subroutine normaliseSDCSDist(tcsob,sdcsen,statebasis,eIncident)
        use totalcs_module
        use state_class
        implicit none
        ...
        real(dp)::eIon, tcsCurr, tics, deltaE, eIncident
    ...
    subroutine calcProbs(sdcsBasis,stateBasis,eOut)
        ...
        do ii=1,sdcsBasis%Nein-1
           ...
           if (eOut .lt. eIncident-eIon) then
              ...
              call getSdcsDistAtEIn(sdcsBasis,eIncident,sdcsDistEn,tcsEn,stateBasis)
