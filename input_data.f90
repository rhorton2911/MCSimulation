module input_data
  ...
  type input
     character*10:: target          ! target label
	 ...
	 character*40:: ionop     ! ionisation option
     ...
     character*50:: DATApath      ! path to the folder with the data on given type of particle
     character*40:: DATApathVcs   ! path to folder with vibrational cross sections for e-H2
     character*40:: DATApathdcsinel  !Path to inelastic dcs files 
     character*40:: DATApathDf
     character*50:: DATApathVcsPseudo  !Path to vibrational pseudostates
     ...
     integer:: NVcsPseudo
     !was length 40
     character*80, pointer, dimension(:) :: filename_tcs => NULL(), filename_addics => NULL(), filename_df => NULL(), filename_sdcs => NULL(), &
     ...
     real(dp):: eV = 27.21138505    !SI to Hartees conversion factor
     real(dp):: bohrRadius = 5.29e-11  !m^2
     real(dp):: eVToSi = 1.60e-19
...
contains
  ...
  subroutine readin( self, nfile,iwrite )
    ...
    real(dp):: eV, en_eV
    logical:: ex
    integer:: iostat_data, itcstype, iaddtcstype, idftype, isdcstype, isdcsnewtype
    character*20:: mode
    character*60::tempDataPath
    ...
    self%DATApathdcsinel= self%DATApath
    self%DATApath = tempDataPath
    ...
    self%DATApathVcs= self%DATApath
    self%DATApath = tempDataPath
    ...
    self%DATApathVcsPseudo= self%DATApath
    self%DATApath = tempDataPath 
...
!
  subroutine read_files_block(self,nopenfile,iwrite,Nfiles,filename_ar)
    ...
    character*80, pointer, dimension(:) :: filename_ar  !=> NULL()
    integer:: i
