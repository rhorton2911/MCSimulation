module state_class
  ...
  type, public:: state
!     private
     character*10:: stlabel    ! state label or '?'
...
contains
  ...
  subroutine init_state(self,stlabel,en,enex,l,m,ipar,spin,ion)
    ...
    character*10, intent(in):: stlabel
  ...
  !
   subroutine set_cs(self,inum,ar_ein,ar_cs,iadd)
    ...
    if(iadd .eq. 0) return
    if(self%enex .eq. 0) return  ! not for elastic scattering 
...
!
  subroutine set_df(self,idf,ar_ein,ar_df)
    ...
    do j=1,iadd
       self%eindf(j) = ar_ein(1) - 0.1*(iadd-j+1)
       self%df(j) = ar_df(1)
    enddo
