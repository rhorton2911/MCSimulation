program main
  ...
  logical:: ex
  integer:: nfile, i,n, nst
  type(basis_totalcs):: tcsbasis
  character*40:: filename
  character*10:: filenamest, enString
  character*4:: stnumber
  integer:: Nmaxall
  ...
  real(dp):: en_incident, eIonBenchmark, intVal, deltaE, cutoffEn
  real(dp):: percentage
  ...
  if(data_in%benchmark) then                    ! True if Ps Benchmark Simulation is to be done
     ...
     do n=1,data_in%Ntcs
        filename = data_in%filename_tcs(n)
	...
	!    call print_tcs(tcsbasis%b(n))
     ...
     call readDics("../DATA_1/DissIonTotal.csv", dicsBasis)
     ...
     call readSdcs(data_in,sdcsBasis,Nenfine,enfine)
     ...
     call constructSDCSDist(tcs,sdcsBasis,statebasis)    
     ...
     if (data_in%debugOp .eq. 1) then
        ...
        call testInputData(tcsbasis, statebasis, eldcs, sdcsBasis, enfine, Nenfine)
     end if
  end if
  ...
  do ii = 1, data_in%totalVariations
    ...
    !Each variation is run data_in%totalSims/data_in%totalVariations times.
     call mcsimulation(data_in,statebasis,sdcsBasis,eldcs,dicsBasis,VarPs,datamcArray(ii))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine testInputData(tcsbasis, statebasis, eldcs, sdcsBasis, enfine, Nenfine)
    use input_data         ! definitions for input data type, construct routine, object data_in
    use state_class        ! defines states (and basis of them) with operations on them
    use totalcs_module     ! reading totalcs files
    use sdcs_module !Handles operations on sdcs
    ...
    logical:: writeData        
     ...
     call get_csatein(stateBasis,en_incident,tcs)
     call getSdcsDistAtEin(sdcsBasis,en_incident,sdcsaten,tcs,stateBasis) 
...
subroutine testTics(sdcsob,stateBasis)
    ...
    do ii = 1, sdcsob%Nein-1
       ...
       if (eIn .gt. eIon) then
          ...
          call getSdcsDistAtEIn(sdcsob,eIn,sdcsatenDist,tcs,stateBasis)
