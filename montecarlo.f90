...
contains
	...
	real(dp):: enex,en ! excitation and ionisation energy (eV)
        real(dp):: enexVib ! vibrational excitation energy
	...
	real(dp):: elEnergyLoss	  ! energy lost by incident particle (eV)
        ...
        real(dp):: minExcEn !Minimum target electron excitation energy
        logical:: indist
	! For Ps Benchmark Simulation
	...
	if (stateNum == 1) then ! elastic collision					
	...
	else if((en < 0) .or. (statebasis%b(stateNum)%ion .eqv. .false.)) then ! excitation collision 
	   ...
	   if(bmode) then
        ...
        else if(((stateNum .gt. 1) .and. (en > 0)) .and. (statebasis%b(stateNum)%ion .eqv. .true.)) then ! ionisation collision 
	...
	real(dp):: df, randNum, en, enIncident
        ...
        if (data_input%benchmarkOp .eq. 1) then
  	   ...
  	   ! Get the dissociation fraction for selected state at current energy
           if (data_input%dicsop .eq. 1) then
              ...
              if ((en <0) .and. (statebasis%b(stateNum)%ion .eqv. .false.) ) then	
                 !excitation collision
	         df = tcs%df(stateNum)
              else if ((en >0) .and. (statebasis%b(stateNum)%ion .eqv. .true.)) then
	...
	if(randNum .lt. df) then
        ...
        if (diss .eqv. .true.) then
           if (statebasis%b(stateNum)%spin .eq. 1) then
              datasim%tripDiss = datasim%tripDiss + 1 
           else if (statebasis%b(stateNum)%spin .eq. 0) then
           ...
           if ((statebasis%b(stateNum)%resolved) .and. (statebasis%b(stateNum)%enex - statebasis%b(stateNum)%dissThresh .gt. 0.0d0)) then
              datasim%dissHeat = statebasis%b(stateNum)%enex - statebasis%b(stateNum)%dissThresh
           end if
        end if
	...
	real(dp):: aveTRes, eIon
        ...
        real(dp):: aveGVibEn  !average primary energy lost in vibrational excitation of ground state 
	...
	if(bmode) then ! Ps Formation Benchmark Run
	   ...
	   write(70,*) 'ave number of dissociations:', aveDissTotal
           ...
           aveDISing = (datamc%singDiss/float(totalSims))/aveIons
           aveDITri = (datamc%tripDiss/float(totalSims))/aveIons
           aveDIb = (datamc%b3SuDiss/float(totalSims))/aveIons
           !aveGVibEn = datamc%   /float(totalSims)
           aveDissHeat = datamc%dissHeat/float(totalSims)
           aveB1SuDiss = (datamc%B1SuDiss/float(totalSims))/aveIons
           aveC1PuDiss = (datamc%C1PuDiss/float(totalSims))/aveIons
           ...
           write(70,*) 'ave dissociations per ion pair (C1Pu): ', aveB1SuDiss
           write(70,*) 'ave dissociations per ion pair (B1Su): ', aveC1PuDiss
...
!
	...
	INTEGER, INTENT(IN) :: stateNum
	type(PsVar), intent(in) :: VarPs
	REAL(dp) :: m_p, eVToJoules, velocity, constant, con2, m
    ...
    subroutine sampleNormal(xToGet, mu, sigma)
        ...
        real(dp), dimension(:), allocatable:: xVec, weights, PVec, func
        real(dp):: dx, randVal
        integer:: ii, kk, numX
    ...
    subroutine calculateErrors(datamcArray, numVars, data_input)
        ...
        real(dp):: uSingletExc, uTripletExc, uRad, uWInv
        ...
        real(dp):: uIonsFin, uExcitesFin, uElasticsFin, uGensFin, uDissFin, uB1SuExcFin, uC1PuExcFin
        real(dp):: uSingletExcFin, uTripletExcFin, uRadFin, uWInvFin
